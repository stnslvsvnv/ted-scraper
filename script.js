/* TED Scraper Frontend - Полная версия с broader defaults и clear button */
...
// Set Default Dates (broader: 2024-01-01 to today for >1000 results)
function setDefaultDates() {
    const today = new Date();
    const fromDate = new Date('2024-01-01');
    const fromStr = fromDate.toISOString().split('T')[0];  // 2024-01-01
    ...
    console.log('Default dates set for demo:', fromStr, 'to', toStr, '— expect total >1000');
}
...
function clearForm() {
    ...
    currentPage = 1;
    console.log('Form cleared — searching with * query');
    performSearch();
}
...
function showNoResults() {
    if (elements.emptyState) {
        elements.emptyState.innerHTML = `
            ...
            <p>Попробуйте: <button type="button" onclick="clearForm()" class="btn btn-sm btn-outline-primary">Очистить фильтры</button> для последних 25, или расширьте даты с 2024-01-01. Добавьте text="construction" или country="DEU".</p>
        `;
